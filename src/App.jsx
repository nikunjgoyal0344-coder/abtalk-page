...
export default function App() {
  ...
  return (
    <div className="app-container">
      ...
      {/* REAL-WORLD STUDENT EDGE-CASE DASHBOARD SANDBOX */}
      <section style={{ padding: '20px 0', position: 'relative' }}>
        <div className="container">
          <div className="reveal reveal-up" style={{ textAlign: 'center', marginBottom: '24px' }}>
            <h2 className="section-title">Student Dashboard</h2>
            <p className="section-subtitle">
              Inspect your progress logs, streak status, and reward history.
            </p>
          </div>
            ...
            {!isDashLoggedIn ? (
              <div style={{
                ...
                <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', maxWidth: '300px', lineHeight: '1.4' }}>
                  Please log in with your ABTalks student account to load your active coding streak data.
                </p>

                <button
                  onClick={handleDashLogin}
                  disabled={isDashLoggingIn}
                  className="btn-primary"
                  style={{ width: '100%', maxWidth: '260px', gap: '8px', marginTop: '8px' }}
                >
                  {isDashLoggingIn ? (
                    <>
                      <Clock size={18} className="animate-glow" />
              ...
              <div style={{ animation: 'fadeInUp 0.6s cubic-bezier(0.16, 1, 0.3, 1) forwards' }}>
                {/* Logout row for dashboard sandbox */}
                <div style={{
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'space-between',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <CheckCircle size={14} />
                    <span>Logged in as <strong>student_abtalks</strong></span>
                  </div>
                  <button 
                    onClick={() => setIsDashLoggedIn(false)} 
                    style={{
                ...
                {/* Edge-case selector menu */}
                <span style={{ fontSize: '0.65rem', color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: 700, display: 'block', marginBottom: '10px', textAlign: 'left' }}>
                ...
                <div style={{
                  ...
                  {[
                    { id: 'fresh', label: 'Day 0 (No Streak)' },
                    { id: 'active', label: 'Active (Day 18)' },
                    { id: 'missed', label: 'Missed Day (Broken)' },
                    { id: 'empty', label: 'Empty Profile' }
                  ].map(item => (
                    <button
                      key={item.id}
                      onClick={() => {
                        setActiveEdgeCase(item.id);
                        if (item.id === 'missed') {
                          setMissedStreakRestored(false);
                        }
                      }}
                      style={{
                        background: activeEdgeCase === item.id ? 'var(--primary-glow)' : 'var(--bg-input)',
                        border: '1px solid ' + (activeEdgeCase === item.id ? 'var(--primary)' : 'rgba(255,255,255,0.02)'),
                        color: activeEdgeCase === item.id ? 'var(--primary)' : 'var(--text-secondary)',
                        padding: '8px 4px',
                        borderRadius: '8px',
                        fontSize: '0.72rem',
                        fontWeight: activeEdgeCase === item.id ? 700 : 500,
                        cursor: 'pointer',
                        transition: 'all 0.3s ease'
                      }}
                    >
                ...
                <div style={{
                  ...
                  {/* CASE A: FRESH START DAY 0 */}
                  {activeEdgeCase === 'fresh' && (
                    <div style={{ animation: 'fadeInUp 0.4s ease forwards', height: '100%' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                        <span className="badge" style={{ margin: 0, background: 'rgba(156,163,175,0.1)', color: '#9CA3AF', borderColor: 'rgba(156,163,175,0.2)' }}>
                          Day 0 • Fresh Start
                        </span>
                        <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>🔥 0 Days</span>
                      </div>
                      ...
                      <p style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', lineHeight: '1.4', marginBottom: '14px' }}>
                        Your coding streak catalog is not initialized yet. Submit your first commit tonight to start your 60-day consistency challenge.
                      </p>

                      {/* Checklist onboarding */}
                      <div style={{ background: 'rgba(255,255,255,0.01)', padding: '10px 12px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.02)' }}>
                        <span style={{ fontSize: '0.62rem', color: 'var(--primary)', fontWeight: 700, textTransform: 'uppercase', display: 'block', marginBottom: '6px' }}>
                          Onboarding Tasks:
                        </span>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', fontSize: '0.72rem', color: 'var(--text-secondary)' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                            <Check size={12} style={{ color: '#10B981' }} />
                            <span>LinkedIn Profile Synced</span>
                          </div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                            <div style={{ width: '12px', height: '12px', borderRadius: '50%', border: '1px solid var(--text-muted)', display: 'inline-block', flexShrink: 0 }} />
                            <span>Push Day 1 Commit before midnight</span>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  {/* CASE B: ACTIVE STREAK DAY 18 */}
                  {activeEdgeCase === 'active' && (
                    <div style={{ animation: 'fadeInUp 0.4s ease forwards' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                        <span className="badge" style={{ margin: 0 }}>
                          Streak Active
                        </span>
                        <span style={{ fontSize: '0.7rem', color: 'var(--primary)', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '4px' }}>
                          <Flame size={12} className="animate-float" />
                          🔥 18 Days
                        </span>
                      </div>
                      ...
                      <p style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', lineHeight: '1.4', marginBottom: '14px' }}>
                        You have submitted 18 consecutive builds after college. Reached the <strong>top 15%</strong> of your cohort!
                      </p>

                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.72rem', color: 'var(--text-muted)' }}>
                        <span>Next Milestone: Day 30</span>
                        <span>12 days left</span>
                      </div>
                      <div style={{ width: '100%', height: '6px', background: 'var(--bg-card)', borderRadius: '3px', marginTop: '6px', overflow: 'hidden' }}>
                        <div style={{ width: '60%', height: '100%', background: 'var(--primary)' }} />
                      </div>
                    </div>
                  )}

                  {/* CASE C: MISSED DAY / BROKEN STREAK */}
                  {activeEdgeCase === 'missed' && (
                    <div style={{ animation: 'fadeInUp 0.4s ease forwards' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                        <span className="badge" style={{ margin: 0, background: missedStreakRestored ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.1)', color: missedStreakRestored ? '#10B981' : '#EF4444', borderColor: missedStreakRestored ? 'rgba(16,185,129,0.2)' : 'rgba(239,68,68,0.2)' }}>
                          {missedStreakRestored ? "Streak Restored" : "Streak Broken"}
                        </span>
                        
                        <span style={{ fontSize: '0.7rem', color: missedStreakRestored ? 'var(--primary)' : '#EF4444', fontWeight: 700 }}>
                          {missedStreakRestored ? "🔥 18 Days" : "⚠️ Streak Reset"}
                        </span>
                      </div>
                      ...
                      {missedStreakRestored ? (
                        <div>
                          ...
                          <p style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', lineHeight: '1.4', marginBottom: '12px' }}>
                            Your streak has been recovered back to Day 18. Keep code commits going tonight to secure the logs!
                          </p>
                          <span style={{ fontSize: '0.65rem', color: 'var(--text-muted)' }}>
                            Remaining streak freezes: <strong>{vibePasses} Vibe Passes left</strong>
                          </span>
                        </div>
                      ) : (
                        <div>
                          ...
                          <p style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', lineHeight: '1.4', marginBottom: '14px' }}>
                            You didn't log any commits before midnight. Your 18-day streak was broken. Use a <strong>Vibe Pass</strong> to restore it instantly!
                          </p>
                  ...
                  {/* CASE D: EMPTY PROFILE STATE */}
                  {activeEdgeCase === 'empty' && (
                    <div style={{ animation: 'fadeInUp 0.4s ease forwards' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                        <span className="badge" style={{ margin: 0, background: 'rgba(239,156,11,0.1)', color: '#F59E0B', borderColor: 'rgba(239,156,11,0.2)' }}>
                          Incomplete Profile
                        </span>
                        <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>⚠️ Empty profile</span>
                      </div>
                      ...
                      <p style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', lineHeight: '1.4', marginBottom: '14px' }}>
                        To fetch and verify commits, you must link your active development handles.
                      </p>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        <button
                          ...
                          style={{
                            ...
                            fontSize: '0.72rem',
                            justifyContent: 'space-between',
                            borderColor: gitSynced ? '#10B981' : 'var(--border-color)',
                            background: gitSynced ? 'rgba(16,185,129,0.05)' : 'transparent'
                          }}
                        >
                          ...
                          {gitSynced ? <CheckCircle2 size={14} style={{ color: '#10B981' }} /> : <ArrowRight size={12} />}
                        </button>

                        <button
                          onClick={() => setLinkedinSynced(true)}
                          className="btn-secondary"
                          style={{
                            padding: '8px 12px',
                            fontSize: '0.72rem',
                            justifyContent: 'space-between',
                            borderColor: linkedinSynced ? '#10B981' : 'var(--border-color)',
                            background: linkedinSynced ? 'rgba(16,185,129,0.05)' : 'transparent'
                          }}
                        >
                          <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                          ...
                          {linkedinSynced ? <CheckCircle2 size={14} style={{ color: '#10B981' }} /> : <ArrowRight size={12} />}
                        </button>
                      </div>
                    </div>
                  )}
      ...
      {/* DAILY STREAK SUBMISSION SIMULATOR (WITH GITHUB LOGIN GATE) */}
      <section style={{ padding: '20px 0', position: 'relative' }}>
        <div className="container">
          <div className="reveal reveal-up" style={{ textAlign: 'center', marginBottom: '24px' }}>
            <span className="badge">Streak Engine</span>
            <h2 className="section-title">Submit Proof of Work</h2>
            <p className="section-subtitle" style={{ marginBottom: '20px' }}>
              Verify daily commits & shares. Log in to authenticate your dev profile.
            </p>
          </div>
          ...
          <div className="glass-card reveal reveal-up delay-100" style={{
            ...
            {/* Confetti particles element specifically for Simulator */}
            {particles.map(p => (
              <div 
                key={p.id}
                className="confetti-particle"
            ...
            {!isLoggedIn ? (
              <div style={{
                ...
                <h4 style={{ fontFamily: 'var(--font-display)', fontSize: '1.25rem', fontWeight: 700 }}>
                  Log In to Submit Streak
                </h4>

                <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', maxWidth: '300px', lineHeight: '1.4' }}>
                  Secure your proof-of-work. Log in with your GitHub account to connect your daily commits to your ABTalks profile card.
                </p>
              ...
              // SIMULATOR MAIN INTERFACE (UNLOCKED)
              <div style={{ animation: 'fadeInUp 0.6s cubic-bezier(0.16, 1, 0.3, 1) forwards' }}>
                {/* Logged in success alert banner */}
                <div style={{
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'space-between',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <CheckCircle size={14} />
                    <span>Logged in as <strong>dev_student</strong></span>
                  </div>
                  <button 
                ...
                <div style={{
                  ...
                  {['frontend', 'backend', 'ai'].map((track) => (
                    <button
                      ...
                      style={{
                        flex: 1,
                        background: simTrack === track ? 'var(--primary)' : 'transparent',
                        color: simTrack === track ? '#FFFFFF' : 'var(--text-secondary)',
                        border: 'none',
                        padding: '8px 4px',
                        borderRadius: '8px',
                        fontSize: '0.72rem',
                ...
                <div style={{
                  ...
                  <span style={{ fontSize: '0.65rem', color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <FileCode size={12} />
                    Day 14 Build Task
                  </span>
                  <p style={{
                    ...
                    {SIMULATOR_PROMPTS[simTrack].prompt}
                  </p>
                </div>

                {/* Checklist interactive check marks */}
                <div style={{ textAlign: 'left', marginBottom: '24px' }}>
                  <span style={{ fontSize: '0.65rem', color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: 700, display: 'block', marginBottom: '12px' }}>
                    Deliverables Check (Tap to complete)
                  </span>
                  
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {/* Checkbox 1: GitHub */}
                    <div 
                      onClick={() => simState === 'idle' && setGitChecked(!gitChecked)}
                      style={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: '10px',
                        cursor: simState === 'idle' ? 'pointer' : 'default',
                        background: gitChecked ? 'rgba(16,185,129,0.05)' : 'transparent',
                        padding: '8px 10px',
                        borderRadius: '8px',
                        border: gitChecked ? '1px solid rgba(16,185,129,0.2)' : '1px solid transparent',
                    ...
                    >
                      <div style={{
                        ...
                        height: '18px',
                        borderRadius: '4px',
                        border: '1.5px solid ' + (gitChecked ? '#10B981' : 'var(--text-muted)'),
                        display: 'flex',
                        alignItems: 'center',
                        justifyContent: 'center',
                        color: '#10B981',
                        background: gitChecked ? 'rgba(16,185,129,0.1)' : 'transparent'
                      }}>
                        {gitChecked && <Check size={12} strokeWidth={3} />}
                      </div>
                      <GithubIcon size={16} style={{ color: 'var(--text-primary)' }} />
                      <span style={{ fontSize: '0.8rem', color: gitChecked ? 'var(--text-primary)' : 'var(--text-secondary)' }}>
                        git commit -m "feat: streak day 14"
                      </span>
                    </div>

                    {/* Checkbox 2: LinkedIn */}
                    <div 
                      onClick={() => simState === 'idle' && setLinkedinChecked(!linkedinChecked)}
                      style={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: '10px',
                        cursor: simState === 'idle' ? 'pointer' : 'default',
                        background: linkedinChecked ? 'rgba(6,182,212,0.05)' : 'transparent',
                        padding: '8px 10px',
                        borderRadius: '8px',
                        border: linkedinChecked ? '1px solid rgba(6,182,212,0.2)' : '1px solid transparent',
                        transition: 'all 0.2s ease'
                      }}
                    >
                      <div style={{
                        ...
                        height: '18px',
                        borderRadius: '4px',
                        border: '1.5px solid ' + (linkedinChecked ? 'var(--secondary)' : 'var(--text-muted)'),
                        display: 'flex',
                        alignItems: 'center',
                        justifyContent: 'center',
                        color: 'var(--secondary)',
                        background: linkedinChecked ? 'rgba(129,140,248,0.1)' : 'transparent'
                      }}>
                        {linkedinChecked && <Check size={12} strokeWidth={3} />}
                      </div>
                      <LinkedinIcon size={16} style={{ color: 'var(--secondary)' }} />
                      <span style={{ fontSize: '0.8rem', color: linkedinChecked ? 'var(--text-primary)' : 'var(--text-secondary)' }}>
                        LinkedIn Post with #ABTalksCodeStreak
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {simState === 'idle' && (
                  <button
                    onClick={startSimulatorSubmission}
                    className="btn-primary"
                    style={{ width: '100%', gap: '8px', opacity: (gitChecked && linkedinChecked) ? 1 : 0.6 }}
                  >
                    <Zap size={18} />
                    Submit Day 14 Streak
                  </button>
                )}
                ...
                {simState === 'verified' && (
                  <div style={{ textAlign: 'center', animation: 'fadeInUp 0.5s ease forwards' }}>
                    <span className="badge" style={{ backgroundColor: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.3)', color: '#10B981', marginBottom: '8px' }}>
                    ...
                    <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginBottom: '16px' }}>
                      GitHub commit indexed. LinkedIn post verified. Day 14/60 log recorded.
                    </p>
